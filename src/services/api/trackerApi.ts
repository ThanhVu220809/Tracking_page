...
async function requestJson<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (!response.ok) {
    ...
    throw new Error(trimmedBody || `Không thể tải dữ liệu từ API (mã ${response.status}).`);
  }
  ...
  if (/tracker worker/i.test(sample)) {
    throw new Error(
      "API trả về văn bản thay vì JSON. Vui lòng kiểm tra lại Worker tracking để các route /api/* trả về JSON."
    );
  }

  throw new Error(
    sample
      ? `API trả về dữ liệu không đúng định dạng cho ${path}: ${sample}`
      : `API trả về phản hồi rỗng cho ${path}.`
  );
}
